...
if( $_POST ){
	...
	foreach( $inputs as $input ){
		list($en, $text) = explode("\t", trim($input));
		if( isset($array[$text]) ) $array[$text] = $en;
	}
	if( $array ) {
		$str = '<?xml version="1.0" encoding="UTF-8"?>'."\n";
		$str .= '<resources>'."\n";
		foreach( $array as $text => $en ) {
			if( $en ) {
				$str .= '<string name="'.$text.'">'.$en.'</string>'."\n";
				$data[$text] = $en;
				
			} else if( strpos($text, '_') !== false ){
				$format = strtr($text, array('_' => ' '));
				
				if( strpos($format, '  ') === false ){
					$a = substr($format, -1, 1);
					if( $a == ' ' ) $format = substr($format, 0, -1).'.';
					$str .= '<string name="'.$text.'">'.$format.'</string>'."\n";
					$data[$text] = $format;
...
<title>Extract En for Titanium Project</title>
</head>
<body>
<?php
if( $nodata ) {
...
if( $data ) {
	...
	foreach( $data as $text => $en ) echo $text."\t".$en."\n";
	echo '</textarea>';
	echo '<hr />';
}
?>
...
<form action="extractEn.php" method="post">
